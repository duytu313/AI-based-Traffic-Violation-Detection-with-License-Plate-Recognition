        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3 pt-2 border-t border-slate-700">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={c.enable_speed_violation_detection}
                onChange={(e) => update({ enable_speed_violation_detection: e.target.checked })}
                className="w-4 h-4 rounded border-slate-600 bg-slate-700 text-blue-500 focus:ring-blue-500"
              />
              <span>⚡ Enable speed violation detection</span>
            </label>

            {c.enable_speed_violation_detection && (
              <div className="space-y-2 pl-6">
                <div>
                  <label className="text-xs text-slate-400">
                    Speed limit: {c.speed_limit || 60} km/h
                  </label>
                  <input
                    type="range"
                    min="20"
                    max="120"
                    step="5"
                    value={c.speed_limit || 60}
                    onChange={(e) => update({ speed_limit: parseInt(e.target.value) })}
                ...
                <p className="text-xs text-slate-500">
                  Speed violations are detected when vehicle speed exceeds the limit. 
                  Requires vehicle tracking (works best with video/webcam streams).
                </p>
              </div>
            )}
          </div>
